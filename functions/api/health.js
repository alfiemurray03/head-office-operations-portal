...
export const onRequestGet = async ({ env }) => {
  ...
  try {
    ...
    const product = await env.DB.prepare("SELECT value_json FROM system_settings WHERE setting_key='system.product_version'").first();
    if (Number(v7?.count || 0) >= 16 && product?.value_json === '"7.0.0"') version7Schema = "ready";
  } catch (error) {
