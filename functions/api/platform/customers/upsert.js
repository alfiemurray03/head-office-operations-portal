  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    try {
      ...
      if (message.includes("external_identity_id")) {
        return await env.DB.prepare("SELECT * FROM customers WHERE external_identity_id=?").bind(identity.externalKey).first();
      }
      throw cause;
    }
  }
...
async function findCustomer(env, platform, input) {
  ...
  const directoryIdentity = await env.DB.prepare(`SELECT c.* FROM customer_directory_identities i
    ...
    WHERE i.provider=? AND i.tenant_id=? AND i.object_id=? LIMIT 1`)
...
async function upsertDirectoryIdentity(env, customer, input, now) {
  ...
  if (existing?.customer_id && existing.customer_id !== customer.id) {
    throw Object.assign(new Error("The Microsoft identity is already linked to another universal customer."), {
  ...
  if (existing) {
    await env.DB.prepare(`UPDATE customer_directory_identities SET customer_id=?,display_name=?,primary_email=?,
      ...
      .bind(customer.id, input.displayName, input.email, input.accountEnabled ? 1 : 0,
        input.accountEnabled ? "active" : "disabled", now, now, existing.id).run();
    return existing.id;
  }
  const id = crypto.randomUUID();
  await env.DB.prepare(`INSERT INTO customer_directory_identities
    ...
    .bind(id, customer.id, CUSTOMER_DIRECTORY_CONNECTOR_ID, CUSTOMER_DIRECTORY_PROVIDER, input.tenantId, input.objectId,
      input.displayName, input.givenName, input.surname, input.email, input.userPrincipalName,
      input.accountEnabled ? 1 : 0, input.accountEnabled ? "active" : "disabled",
      input.createdAt || null, now, now, now, now).run();
  return id;
}

async function upsertPlatformAccount(env, platform, customer, platformCustomerId, status, now) {
  const byCustomer = await env.DB.prepare("SELECT id FROM customer_platform_accounts WHERE customer_id=? AND platform_id=? LIMIT 1")
  ...
  if (byCustomer) {
    await env.DB.prepare(`UPDATE customer_platform_accounts SET external_account_id=?,status=?,last_synced_at=? WHERE id=?`)
      .bind(platformCustomerId, status, now, byCustomer.id).run();
...
export const onRequestPost = async context => {
  ...
  const input = {
    tenantId: cleanText(body.entraTenantId, 100),
    objectId: cleanText(body.entraObjectId, 100),
    platformCustomerId: cleanText(body.platformCustomerId, 160),
    displayName: cleanText(body.displayName, 160),
    givenName: cleanText(body.givenName, 100) || null,
    surname: cleanText(body.surname, 100) || null,
    email: cleanText(body.email, 254).toLowerCase(),
    userPrincipalName: cleanText(body.userPrincipalName, 254) || null,
  ...
  try {
    ...
    if (found.match === "identity_conflict") {
      return error("CUSTOMER_IDENTITY_REVIEW_REQUIRED", "The verified email belongs to a customer linked to a different Microsoft identity. Head Office review is required.", 409, {
    ...
    if (!customer) {
      ...
      const status = input.accountEnabled ? input.accountStatus : "suspended";
      await context.env.DB.prepare(`UPDATE customers SET display_name=?,verified_email=?,
        external_identity_id=COALESCE(external_identity_id,?),account_status=CASE WHEN account_status IN ('pending','active','suspended') THEN ? ELSE account_status END,
        last_activity_at=?,updated_at=? WHERE id=?`)
        .bind(input.displayName, input.email, input.externalKey, status, now, now, customer.id).run();
    ...
    const restrictions = await context.env.DB.prepare(`SELECT restriction_type,scope,status,review_at,expires_at
      FROM restrictions WHERE customer_id=? AND status='active' AND (expires_at IS NULL OR expires_at>?) ORDER BY applied_at DESC`)
    ...
    await platformAudit(context.env, auth.platform, created ? "customer.automatic_create" : "customer.automatic_upsert", "customer", customer.id, {
      ...
      metadata: { match: found.match, identityId, platformAccountId, platformCustomerId: input.platformCustomerId }
    });
    ...
    return json({
      ...
      created,
      matchedBy: found.match,
      enforcement: {
        action: restrictions.results.length ? "review_controls" : "allow",
