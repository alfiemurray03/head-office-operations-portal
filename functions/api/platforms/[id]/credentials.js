import { audit, cleanText, error, json, requireSession, sha256 } from "../../../_shared.js";
...
export const onRequestPost = async context => {
  ...
  const scopes = Array.isArray(body.scopes) ? [...new Set(body.scopes.map(v => cleanText(v, 40)).filter(v => permittedScopes.has(v)))] : [];
  ...
  await context.env.DB.prepare(`INSERT INTO platform_api_credentials
    (id,platform_id,key_prefix,name,secret_hash,scopes_json,status,created_by,created_at)
    VALUES (?,?,?,?,?,?,'active',?,?)`)
    .bind(id, platform.id, keyPrefix, name, await sha256(token), JSON.stringify(scopes), auth.session.sub, now).run();
  await context.env.DB.prepare("UPDATE platforms SET status='active',updated_at=? WHERE id=?").bind(now, platform.id).run();
  await audit(context.env, auth.session, "platform.credential.create", "platform_api_credential", id, {
    label: "Platform API credential generated", reference: keyPrefix, metadata: { platformId, scopes }
  });
...
export const onRequestDelete = async context => {
  ...
  await audit(context.env, auth.session, "platform.credential.revoke", "platform_api_credential", credentialId, {
    label: "Platform API credential revoked", reference: existing.key_prefix
  });
  return json({ ok: true });
};
