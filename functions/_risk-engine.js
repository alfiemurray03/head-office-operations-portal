...
function inferCategory(eventType) {
  ...
  return ({ auth: "authentication", identity: "identity", payment: "payment", refund: "refund", chargeback: "dispute", dispute: "dispute", account: "account", data: "data", system: "system", admin: "administration", complaint: "complaint" })[prefix] || "general";
}
...
function sanitiseAttributes(value) {
  ...
  for (const [key, item] of Object.entries(value).slice(0, 60)) {
    ...
    if (!safeKey) continue;
    if (["string","number","boolean"].includes(typeof item) || item === null) clean[safeKey] = typeof item === "string" ? cleanText(item, 500) : item;
  }
  return clean;
}
...
async function ruleApplies(env, rule, event, attributes) {
  ...
  if (rule.code === "IMPOSSIBLE_TRAVEL") {
    ...
    const prior = await env.DB.prepare(`SELECT country_code FROM security_events
      WHERE customer_id=? AND event_type='auth.succeeded' AND occurred_at>=? AND country_code IS NOT NULL
      ORDER BY occurred_at DESC LIMIT 1`).bind(event.customerId, cutoff).first();
  ...
  if (rule.code === "PAYMENT_METHOD_SHARING") {
    ...
    return Number(row?.count || 0) + (event.customerId ? 1 : 0) >= Number(rule.threshold_count || 3);
  }
  ...
  if (event.customerId) { clauses.push("customer_id=?"); bindings.push(event.customerId); }
  else if (event.platformId) { clauses.push("platform_id=?"); bindings.push(event.platformId); }
  const row = await env.DB.prepare(`SELECT COUNT(*) count FROM security_events WHERE ${clauses.join(" AND ")}`).bind(...bindings).first();
  return Number(row?.count || 0) + 1 >= Number(rule.threshold_count);
}
...
async function writeAudit(env, actor, action, entityType, entityId, referenceValue, metadata = {}) {
  ...
  await env.DB.prepare(`INSERT INTO audit_events
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(crypto.randomUUID(),new Date().toISOString(),actorType,actorId,actorName,action,action.replaceAll("."," "),entityType,entityId,referenceValue,JSON.stringify(metadata)).run();
}

async function upsertAlert(env, event, assessment, matchedRules) {
  if (assessment.score < 30) return null;
  const cutoff = new Date(Date.now() - 24 * 60 * 60_000).toISOString();
  const existing = await env.DB.prepare(`SELECT id,alert_reference,occurrence_count,risk_score FROM security_alerts
  ...
  if (existing) {
    await env.DB.prepare(`UPDATE security_alerts SET occurrence_count=occurrence_count+1,last_detected_at=?,
      ...
      .bind(now,assessment.score,assessment.riskLevel,assessment.enforcementLevel,assessment.severity,
        assessment.dataClassification,assessment.confidentialityLevel,summary,assessment.recommendedAction,now,existing.id).run();
  ...
  await env.DB.prepare(`INSERT INTO security_alerts
    ...
    .bind(id,alertReference,event.customerId,event.platformId,event.caseId,event.category,title,summary,
      ...
      assessment.dataClassification,assessment.confidentialityLevel,now,now,assessment.recommendedAction,now,now).run();
  return { id, reference: alertReference, updated: false };
}

async function createIncidentIfRequired(env, event, assessment, alert, actor) {
  const incidentRequired = assessment.score >= 85 || ["data.unauthorised_access","data.exfiltration_suspected","data.loss_reported","system.ransomware_detected","account.takeover_suspected"].includes(event.eventType);
  if (!incidentRequired) return null;
  const id = crypto.randomUUID();
  const incidentReference = reference("INC");
  const now = new Date().toISOString();
  const isData = event.category === "data" || event.eventType.startsWith("data.");
  const deadline = isData ? new Date(Date.parse(now) + 72 * 60 * 60_000).toISOString() : null;
  ...
  await env.DB.prepare(`INSERT INTO incident_timeline
    ...
    VALUES (?,?,'detection',?,?,?, ?,?)`)
    .bind(crypto.randomUUID(),id,"Incident automatically raised by the risk engine.",JSON.stringify({ eventReference: event.reference, alertReference: alert?.reference || null }),actor.id || "risk-engine",now,now).run();
  if (isData) {
  ...
  if (alert?.id) await env.DB.prepare("UPDATE security_alerts SET incident_id=?,updated_at=? WHERE id=?").bind(id,now,alert.id).run();
  ...
  await env.DB.prepare(`INSERT INTO operations_tasks
    ...
    .bind(taskId,reference("TSK"),"security","incident_triage",event.customerId,id,
...
export async function ingestSecurityEvent(env, input, actor = {}) {
  ...
  if (!/^[a-z0-9_-]+\.[a-z0-9_.-]+$/.test(eventType)) throw Object.assign(new Error("A valid namespaced eventType is required."), { status: 400, code: "INVALID_EVENT_TYPE" });
  ...
  if (customerReference && !customer) throw Object.assign(new Error("The referenced universal customer was not found."), { status: 404, code: "CUSTOMER_NOT_FOUND" });
  const platformId = cleanNullableText(input.platformId || actor.platformId,100);
  if (platformId && !await env.DB.prepare("SELECT id FROM platforms WHERE id=?").bind(platformId).first()) throw Object.assign(new Error("The referenced platform was not found."), { status: 404, code: "PLATFORM_NOT_FOUND" });
  ...
  const dedupeKey = cleanNullableText(input.dedupeKey || (input.externalEventId ? `${actor.id || platformId || "source"}:${input.externalEventId}` : null),240);
  if (dedupeKey) {
    const duplicate = await env.DB.prepare("SELECT id,event_reference,risk_score,risk_level,enforcement_level FROM security_events WHERE dedupe_key=?").bind(dedupeKey).first();
  ...
  const event = {
    id, reference: eventReference, eventType, category, customerId: customer?.id || null,
    platformId, caseId: cleanNullableText(input.caseId,100), occurredAt,
    ...
    ipHash: cleanNullableText(input.ipHash,128), deviceHash: cleanNullableText(input.deviceHash,128),
  ...
  await env.DB.prepare(`INSERT INTO security_events
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'processing',?)`)
    .bind(id,eventReference,cleanText(actor.type || input.sourceType || "staff",40),cleanNullableText(actor.id || input.sourceId,120),
      cleanNullableText(input.externalEventId,160),eventType,category,event.customerId,platformId,event.caseId,occurredAt,receivedAt,
      event.amountMinor,event.currency,event.countryCode,event.ipHash,event.deviceHash,event.paymentFingerprintHash,
      JSON.stringify(attributes),dedupeKey,receivedAt).run();

  const rules = await env.DB.prepare("SELECT * FROM detection_rules WHERE enabled=1 AND event_type=? ORDER BY base_score DESC").bind(eventType).all();
  const matched = [];
  let score = 0;
  for (const rule of rules.results) {
    ...
    matched.push(rule);
    score += points;
    await env.DB.prepare(`INSERT INTO risk_signals (id,event_id,rule_code,points,label,rationale,created_at)
      VALUES (?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),id,rule.code,points,rule.name,rule.description,receivedAt).run();
  }
  ...
  const enforcementLevel = maxCode(matched.map(rule => rule.recommended_enforcement),ENFORCEMENT_RANK,riskLevel === "R4" ? "A5" : riskLevel === "R3" ? "A3" : riskLevel === "R2" ? "A2" : riskLevel === "R1" ? "A1" : "A0");
  ...
  const recommendedAction = ({ A0:"Allow",A1:"Allow and monitor",A2:"Require step-up verification",A3:"Hold for manual approval",A4:"Apply a targeted restriction",A5:"Apply an urgent protective freeze" })[enforcementLevel];
  const assessment = { score,riskLevel,enforcementLevel,severity,dataClassification,confidentialityLevel,recommendedAction };
  await env.DB.prepare(`UPDATE security_events SET processing_status='processed',risk_score=?,risk_level=?,enforcement_level=?,
    data_classification=?,confidentiality_level=? WHERE id=?`)
  ...
  await writeAudit(env,actor,"risk.event.processed","security_event",id,eventReference,{ eventType,category,score,riskLevel,enforcementLevel,alertReference:alert?.reference || null,incidentReference:incident?.reference || null });
  return { duplicate:false,event:{ id,reference:eventReference,eventType,category,...assessment },signals:matched.map(rule => ({ code:rule.code,name:rule.name,points:Number(rule.base_score) })),alert,incident };
}
