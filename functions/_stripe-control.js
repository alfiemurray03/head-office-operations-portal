...
const STRIPE_SCHEMA = [
  ...
  "CREATE INDEX IF NOT EXISTS idx_stripe_division_events ON stripe_division_webhook_events(connector_code,received_at DESC)",
  "CREATE INDEX IF NOT EXISTS idx_stripe_division_links ON stripe_division_customer_links(connector_code,stripe_customer_id)",
  "CREATE INDEX IF NOT EXISTS idx_stripe_division_payments ON stripe_division_payment_records(connector_code,occurred_at DESC)",
  "CREATE INDEX IF NOT EXISTS idx_stripe_division_orders ON stripe_division_order_records(connector_code,occurred_at DESC)",
  "CREATE INDEX IF NOT EXISTS idx_stripe_division_subscriptions ON stripe_division_subscription_records(connector_code,updated_at DESC)"
];
...
function normaliseConnectorKey(value) {
  const key = cleanText(value, 80).toLowerCase().replaceAll("_", "-").replaceAll(" ", "-");
  if (["profilecentre", "profile-center", "profilecenter"].includes(key)) return "profile-centre";
  return key;
}

export function resolveStripeConnector(env, value) {
  const key = normaliseConnectorKey(value);
  const blueprint = CONNECTOR_BLUEPRINTS.find(item => item.slug === key || normaliseConnectorKey(item.code) === key);
  if (!blueprint) {
...
function connectorMode(secretKey) {
  if (String(secretKey).startsWith("sk_live_") || String(secretKey).startsWith("rk_live_")) return "live";
  if (String(secretKey).startsWith("sk_test_") || String(secretKey).startsWith("rk_test_")) return "test";
  return "unknown";
}
...
function parseStripeSignature(value) {
  ...
  for (const item of String(value || "").split(",")) {
    const [key, token] = item.trim().split("=", 2);
...
async function resolveCustomer(env, connector, object) {
  ...
  if (stripeId) {
    const linked = await env.DB.prepare(`SELECT c.id,c.customer_number,c.verified_email,c.display_name
      ...
      WHERE l.connector_code=? AND l.stripe_customer_id=?`).bind(connector.code, stripeId).first();
    if (linked) return linked;
  }
  const email = emailFromObject(object);
  if (!email) return null;
  const matches = await env.DB.prepare("SELECT id,customer_number,verified_email,display_name FROM customers WHERE lower(verified_email)=? LIMIT 2").bind(email).all();
...
async function upsertStripeCustomer(env, connector, object, customer, livemode) {
  ...
  await env.DB.prepare(`INSERT INTO stripe_division_customer_links
    ...
    ON CONFLICT(connector_code,stripe_customer_id) DO UPDATE SET
      ...
      name=COALESCE(excluded.name,stripe_division_customer_links.name),
    ...
    .bind(crypto.randomUUID(), connector.code, stripeId, customer?.id || null, customer?.customer_number || ucnFromObject(object),
      emailFromObject(object), cleanText(object?.name || object?.customer_details?.name, 160) || null,
      livemode ? 1 : 0, now, now).run();
}
...
async function storePayment(env, connector, event, object, customer) {
  ...
  await env.DB.prepare(`INSERT INTO stripe_division_payment_records
    ...
    ON CONFLICT(connector_code,stripe_object_id) DO UPDATE SET
      ...
      customer_id=COALESCE(excluded.customer_id,stripe_division_payment_records.customer_id),
      customer_number=COALESCE(excluded.customer_number,stripe_division_payment_records.customer_number),
      status=excluded.status,amount_minor=COALESCE(excluded.amount_minor,stripe_division_payment_records.amount_minor),
      currency=COALESCE(excluded.currency,stripe_division_payment_records.currency),
      description=COALESCE(excluded.description,stripe_division_payment_records.description),
      receipt_email=COALESCE(excluded.receipt_email,stripe_division_payment_records.receipt_email),
      updated_at=excluded.updated_at,metadata_json=excluded.metadata_json`)
    .bind(crypto.randomUUID(), connector.code, object.id, object.object || "stripe_object", event.type,
      customer?.id || null, customer?.customer_number || ucnFromObject(object), stripeCustomerId(object), connector.code,
      cleanText(object.status || event.type.split(".").at(-1), 80), Number.isFinite(Number(amount)) ? Number(amount) : null,
      cleanText(object.currency, 3).toUpperCase() || null, cleanText(object.description, 500) || null,
      emailFromObject(object), isoFromUnix(object.created, event.created ? isoFromUnix(event.created) : now), now,
...
async function storeOrder(env, connector, object, customer) {
  ...
  await env.DB.prepare(`INSERT INTO stripe_division_order_records
    ...
    ON CONFLICT(connector_code,stripe_object_id) DO UPDATE SET
      ...
      customer_number=COALESCE(excluded.customer_number,stripe_division_order_records.customer_number),
      status=excluded.status,payment_status=excluded.payment_status,amount_total_minor=excluded.amount_total_minor,
      currency=excluded.currency,customer_email=COALESCE(excluded.customer_email,stripe_division_order_records.customer_email),
      updated_at=excluded.updated_at,metadata_json=excluded.metadata_json`)
    .bind(crypto.randomUUID(), connector.code, object.id, customer?.id || null,
      ...
      Number.isFinite(Number(object.amount_total)) ? Number(object.amount_total) : null,
      cleanText(object.currency, 3).toUpperCase() || null, emailFromObject(object), isoFromUnix(object.created), now,
      JSON.stringify({ ...metadataOf(object), headOfficeStripeConnector: connector.code })).run();
}
...
async function storeSubscription(env, connector, object, customer) {
  ...
  await env.DB.prepare(`INSERT INTO stripe_division_subscription_records
    ...
    ON CONFLICT(connector_code,stripe_subscription_id) DO UPDATE SET
      ...
      customer_number=COALESCE(excluded.customer_number,stripe_division_subscription_records.customer_number),
      status=excluded.status,price_id=COALESCE(excluded.price_id,stripe_division_subscription_records.price_id),
      product_id=COALESCE(excluded.product_id,stripe_division_subscription_records.product_id),
      quantity=COALESCE(excluded.quantity,stripe_division_subscription_records.quantity),
    ...
    .bind(crypto.randomUUID(), connector.code, object.id, customer?.id || null,
      ...
      isoFromUnix(object.current_period_start, null), isoFromUnix(object.current_period_end, null),
      object.cancel_at_period_end ? 1 : 0, object.canceled_at ? isoFromUnix(object.canceled_at, null) : null,
      isoFromUnix(object.created), now, JSON.stringify({ ...metadataOf(object), headOfficeStripeConnector: connector.code })).run();
}
...
export async function processStripeWebhookEvent(env, connector, event, rawBody) {
  ...
  if (!event?.id || !event?.type || !object?.id) {
    throw Object.assign(new Error("Stripe sent an incomplete Snapshot event payload."), { code: "STRIPE_EVENT_INVALID", status: 400 });
  }
  const eventKey = `${connector.code}:${event.id}`;
  const receivedAt = new Date().toISOString();
  const payloadHash = await sha256(rawBody);
  const existing = await env.DB.prepare("SELECT processing_status FROM stripe_division_webhook_events WHERE event_key=?").bind(eventKey).first();
  if (existing) return { duplicate: true, status: existing.processing_status, connectorCode: connector.code };

  await env.DB.prepare(`INSERT INTO stripe_division_webhook_events
    ...
    VALUES (?,?,?,?,?,?,?,?, 'received',?,?)`)
    .bind(eventKey, connector.code, event.id, event.type, event.livemode ? 1 : 0,
      cleanText(event.api_version, 80) || null, object.id, stripeCustomerId(object), payloadHash, receivedAt).run();

  try {
    const customer = await resolveCustomer(env, connector, object);
    ...
    await env.DB.prepare("UPDATE stripe_division_webhook_events SET processing_status=?,processed_at=? WHERE event_key=?")
    ...
    await env.DB.prepare("UPDATE stripe_division_webhook_events SET processing_status='failed',processed_at=?,error_message=? WHERE event_key=?")
...
export async function testStripeApiConnection(env, division) {
  ...
  const response = await fetch("https://api.stripe.com/v1/account", {
    headers: { Authorization: `Bearer ${connector.secretKey}` }
  });
...
export async function stripeOperationalStatus(env, origin) {
  ...
  for (const summary of stripeConnectorCatalog(env, origin)) {
    const [events, counts] = await env.DB.batch([
      env.DB.prepare(`SELECT connector_code,event_id,event_type,livemode,api_version,object_id,processing_status,received_at,processed_at,error_message
        FROM stripe_division_webhook_events WHERE connector_code=? ORDER BY received_at DESC LIMIT 20`).bind(summary.code),
      env.DB.prepare(`SELECT
        (SELECT COUNT(*) FROM stripe_division_payment_records WHERE connector_code=?) payments,
        (SELECT COUNT(*) FROM stripe_division_order_records WHERE connector_code=?) orders,
        (SELECT COUNT(*) FROM stripe_division_subscription_records WHERE connector_code=?) subscriptions,
        (SELECT COUNT(*) FROM stripe_division_webhook_events WHERE connector_code=? AND processing_status='failed') failed_events`)
        .bind(summary.code, summary.code, summary.code, summary.code)
    ]);
    connectors.push({
      ...summary,
      counts: counts.results?.[0] || { payments: 0, orders: 0, subscriptions: 0, failed_events: 0 },
      recentEvents: events.results || []
    });
  }
  const totals = connectors.reduce((result, connector) => {
    for (const key of ["payments", "orders", "subscriptions", "failed_events"]) {
      result[key] += Number(connector.counts?.[key] || 0);
    }
    return result;
...
export async function stripeDivisionRecords(env, division, query = "", limit = 100) {
  await ensureStripeControlSchema(env);
  const connector = resolveStripeConnector(env, division);
  const cleanQuery = cleanText(query, 120);
  const safeLimit = Math.max(10, Math.min(Number(limit) || 100, 250));
  const search = `%${cleanQuery.replaceAll("%", "\\%").replaceAll("_", "\\_")}%`;
  const [payments, orders, subscriptions] = await env.DB.batch([
    env.DB.prepare(`SELECT p.*,c.display_name customer_name,c.verified_email customer_email
      FROM stripe_division_payment_records p LEFT JOIN customers c ON c.id=p.customer_id
      WHERE p.connector_code=? AND (?='' OR p.stripe_object_id LIKE ? ESCAPE '\\' OR p.customer_number LIKE ? ESCAPE '\\'
        OR c.display_name LIKE ? ESCAPE '\\' OR p.receipt_email LIKE ? ESCAPE '\\')
      ORDER BY p.occurred_at DESC LIMIT ?`).bind(connector.code, cleanQuery, search, search, search, search, safeLimit),
    env.DB.prepare(`SELECT o.*,c.display_name customer_name,c.verified_email customer_email
      FROM stripe_division_order_records o LEFT JOIN customers c ON c.id=o.customer_id
      WHERE o.connector_code=? AND (?='' OR o.stripe_object_id LIKE ? ESCAPE '\\' OR o.customer_number LIKE ? ESCAPE '\\'
        OR c.display_name LIKE ? ESCAPE '\\' OR o.customer_email LIKE ? ESCAPE '\\')
      ORDER BY o.occurred_at DESC LIMIT ?`).bind(connector.code, cleanQuery, search, search, search, search, safeLimit),
    env.DB.prepare(`SELECT s.*,c.display_name customer_name,c.verified_email customer_email
      FROM stripe_division_subscription_records s LEFT JOIN customers c ON c.id=s.customer_id
      WHERE s.connector_code=? AND (?='' OR s.stripe_subscription_id LIKE ? ESCAPE '\\' OR s.customer_number LIKE ? ESCAPE '\\'
        OR c.display_name LIKE ? ESCAPE '\\' OR s.stripe_customer_id LIKE ? ESCAPE '\\')
      ORDER BY s.updated_at DESC LIMIT ?`).bind(connector.code, cleanQuery, search, search, search, search, safeLimit)
  ]);
  return {
    connector: { slug: connector.slug, code: connector.code, name: connector.name },
