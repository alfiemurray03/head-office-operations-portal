...
export const SYSTEM_SETTING_DEFINITIONS = new Map([
  ...
  ["integrations.stripe_planyx_enabled", { group: "integrations", defaultValue: true, validate: boolean, description: "Allows Planyx Stripe connection, reconciliation and webhook processing." }],
  ["integrations.stripe_profile_centre_enabled", { group: "integrations", defaultValue: true, validate: boolean, description: "Allows Profile Centre Stripe connection, reconciliation and webhook processing." }],
  ...
  ["automation.stripe_reconciliation_enabled", { group: "automation", defaultValue: true, validate: boolean, description: "Allows scheduled Stripe reconciliation for both divisions." }],
...
export async function ensureSystemSettingsReady(env) {
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS service_test_runs (
  ...
  await env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_service_test_runs_service ON service_test_runs(service_code,started_at DESC)").run();
  const now = new Date().toISOString();
  for (const [key, definition] of SYSTEM_SETTING_DEFINITIONS) {
    await env.DB.prepare(`INSERT INTO system_settings(setting_key,setting_group,value_json,description,updated_by,updated_at)
      VALUES (?,?,?,?,?,?) ON CONFLICT(setting_key) DO NOTHING`)
      .bind(key, definition.group, JSON.stringify(definition.defaultValue), definition.description, "system", now).run();
  }
}
