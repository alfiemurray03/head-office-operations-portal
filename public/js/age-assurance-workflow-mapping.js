...
(() => {
  ...
  function attach() {
    if (typeof renderSettings !== 'function' || typeof api !== 'function') {
      window.setTimeout(attach, 100);
      return;
    }
    if (renderSettings.__ageWorkflowMappingsAttached) return;
    const previousRenderSettings = renderSettings;
    const enhanced = async function renderSettingsWithAgeWorkflowMappings() {
    ...
    enhanced.__ageWorkflowMappingsAttached = true;
