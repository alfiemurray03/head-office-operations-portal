...
The migration deliberately installs the service without starting enforcement:
...
No customer access decision changes until both the master switch and the relevant platform deployment are enabled.
...
The service exposes separate governed controls for Planyx and Profile Centre, including deployment state, minimum age, threshold-validation confirmation and evidence validity.

The threshold-validation switch must be set only after the Didit Age Gate workflow has been tested against the relevant threshold. This prevents a workflow tested for 16+ from being assumed to establish 18+ eligibility.
...
The response includes the website deployment state, minimum age, readiness state, whether enforcement has started and the fixed staff-account exclusion.
...
- the threshold has not been validated;
- Didit is not configured; or
- customer disclosure/consent has not been recorded.

A successful response returns the hosted Didit verification URL and identifiers required by the connected website. The Didit API key remains server-side in Head Office.

### Authoritative access check

Connected websites continue to use the central platform access-decision endpoint. The response now includes `ageAssurance` and can return:
